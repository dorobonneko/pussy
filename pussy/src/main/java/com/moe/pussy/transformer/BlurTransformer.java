...
public class BlurTransformer implements Transformer
{
	...
	{
		Bitmap out_bitmap=bp.getBitmap(source.getWidth()/4,source.getHeight()/4,Bitmap.Config.ARGB_8888);
		...
		rs.destroy();*/
		fastblur(out_bitmap,level);
		canvas.setBitmap(source);
		canvas.scale(4,4);
		canvas.drawBitmap(out_bitmap,0,0,null);
		if(out_bitmap!=source)
		bp.recycle(out_bitmap);
		//return out_bitmap;
		return source;
	}
	...
	public static Bitmap fastblur(Bitmap sentBitmap, int radius) {
        ...
        Bitmap bitmap = sentBitmap;// sentBitmap.copy(sentBitmap.getConfig(), true);
